...
Later we groupped private universities in Ýstanbul by their program names and ordered by max scores in descending order to compare their success orders.
```{r}
...
```{r}
dpr = dtr %>% filter(city=='ÝSTANBUL') %>% group_by(university_name) %>% summarise(sayi =n(), totalquota=sum(general_quota), totalplacement=sum(general_placement), fillrate =sum(totalplacement*100/totalquota))  %>% group_by(university_name) %>%  arrange(desc(fillrate))
