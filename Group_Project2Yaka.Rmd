...
ggplot(data = depLeftSatis, aes( x = reorder(departments, -LeftRate), y = LeftRate)) +
    theme(axis.text.x = element_text(angle = 60, hjust=1))+
...
ggplot(data = depLeftSatis, aes( x = departments, y = LeftRate, fill=salary)) +
    theme(axis.text.x = element_text(angle = 60, hjust=1)) +
...
p9 <- ggplot(d, aes(x=d$departments,fill=as.character(d$left) ))+
  theme(axis.text.x = element_text(angle = 60, hjust=1))+
  geom_bar() +
  labs(x="Departments", y="Employee Count")

p10 <- ggplot(d, aes(x=d$salary,fill=as.character(d$left) ))+
      geom_bar() +
      labs(x="Salary", y ="Employee Count",fill="aa")
...
ggcorrplot(CorrMat, hc.order = TRUE, 
           ...
           ggtheme=theme_bw)
```
Let's see correlation matrix for people who quit, already.
```{r}
