...
Now, we are going to use this correlation matrix to make principal component analysis. Before that we need to transform salaries to numeric values to see their effect on left rate. We need also to scale our data to avoid the dominance of the columns with high variance.
```{r}
names(dLeft)
...
<!--                 choices = c("accounting","hr","IT","management","marketing","product_mng","RandD","sales","support","technical"),selected="sales"), -->
<!--   selectInput("salary", label = "Salary:", -->
<!--                 choices = c("low","medium","high"),selected="low"), -->
<!--     sliderInput("stlvl", label = "Satisfaction level:", -->
<!--                 min= 0.05, max=1.0, value = c(0.05,1.0), step=0.05) -->
<!--   ) -->
...
<!-- ggplot(d %>% filter(departments==as.character( input$dept) & satisfaction_level>=as.numeric(input$stlvl[1]) & satisfaction_level<=as.numeric(input$stlvl[2]) & salary==as.character(input$salary)),aes(satisfaction_level,average_montly_hours))+ -->

<!--   geom_point()+ -->

<!-- # geom_point(aes(col=factor(d%>% filter(departments==as.character( input$dept) %>% select(left)))))+ -->
