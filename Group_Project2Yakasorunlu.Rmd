...
```{r}
depLeftSatis <- d %>% group_by(departments) %>% summarise(Total =n(),LeftCount = sum(as.integer(left=="Yes"), Satisfaction = mean(satisfaction_level) ) %>% mutate(LeftRate = LeftCount/Total) 
...
```{r}
depLeftSatis <- d %>% group_by(departments,salary) %>% summarise(Total =n(),LeftCount = sum(as.integer(left=="Yes"), Satisfaction = mean(satisfaction_level) ) %>% mutate(LeftRate = LeftCount/Total)
...
dLeft_pca <- dLeft  %>% mutate(salary_Num = ifelse(salary == "low", 0, ifelse(salary == "medium", 1, 2))) %>% select(-salary,-departments)
 ...
 survey_pca1 <-princomp(as.matrix(dLeft_pca[,1:8],scale=FALSE,cor=T))

 survey_pca <-princomp(as.matrix(dLeft_pca[,1:8],scale=TRUE,cor=T))

summary(survey_pca1,loadings=TRUE)
