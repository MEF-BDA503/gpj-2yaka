...
Now, let's look at the categorical variables (i.e salary, department name)
```{r}
p9 <- ggplot(d, aes(x=d$departments,fill=as.character(d$left) ))+theme(axis.text.x = element_text(angle = 60, hjust=1))+
      geom_bar()
...
```
Now let' find departmentwise job termination ratios by department:
```{r}
d %>% group_by(departments)  %>% summarise(Left_avg =mean(left)) %>% round(digits=2)
```

```{r}

 ggplot(data=d, aes(x=departments, y=mean(satisfaction_level))) +
